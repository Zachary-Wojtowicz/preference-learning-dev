...
3. **Prototype Config Block** (BL_1HuzArhzLYFnRWu, QID6+QID7) — multiple choice selectors for condition and domain that set embedded data via QuestionJS. **This block is for prototype testing only; remove for production.**
...
#### Prototype Config Questions (QID6, QID7)

These are MC questions with QuestionJS that set embedded data:

**QID6 — Condition selector** (block BL_1HuzArhzLYFnRWu):
...
**QID7 — Domain selector** (same block):
